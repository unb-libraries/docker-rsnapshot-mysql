...
for DB in $DATABASES; do
  $MYSQLDUMP -h MYSQL_HOSTNAME -P MYSQL_PORT --force --events --ignore-table=mysql.event --opt --user=MYSQL_USER_NAME -pMYSQL_USER_PASSWORD --databases $DB | gzip > "/tmp/$DB.gz"
  mv "/tmp/$DB.gz" "./$DB.gz"
done
